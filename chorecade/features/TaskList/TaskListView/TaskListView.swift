...
class TaskListView: UIView {
    ...
    lazy var addNewTaskButton = Components.getButton(
    ...
    lazy var tasksTableView: UITableView = {
        ...
        tableView.register(
            ...
            forCellReuseIdentifier: "taskList-cell"
        )
        tableView.register(
            EmptyTableViewCell.self,
            forCellReuseIdentifier: "empty-list-cell"
        )
    ...
    func reloadTasksForCurrentGroup() {
        guard let selectedGroup = self.currentSelectedGroup else {
            print(
                "DEBUG: Nenhum grupo selecionado, não é possível recarregar tarefas."
            )
            return
        }

        if let updatedTasks = Repository.tasksModelsByGroup[
            String(describing: selectedGroup.id)
        ] {
            self.tasksByGroup = updatedTasks
            self.tasksTableView.reloadData()
            self.loadingOverlay?.hide()
            self.loadingOverlay = nil
        } else {
            print("Erro ao buscar tarefas para o grupo")
        }
    ...
    override func observeValue(
        ...
        if keyPath == "contentSize" {
            ...
            let heightConstraint = tasksTableView.heightAnchor.constraint(
                equalToConstant: tasksTableView.contentSize.height
            )
...
extension TaskListView: ViewCodeProtocol {
    ...
    func setupConstraints() {
        NSLayoutConstraint.activate([
            ...
            tasksStack.topAnchor.constraint(
                equalTo: addNewTaskButton.bottomAnchor,
                constant: 24
            ),
            tasksStack.leadingAnchor.constraint(
                equalTo: contentView.leadingAnchor,
                constant: 16
            ),
            tasksStack.trailingAnchor.constraint(
                equalTo: contentView.trailingAnchor,
                constant: -16
            ),
            tasksStack.bottomAnchor.constraint(
...
extension TaskListView: AddNewTaskModalDelegate {
    ...
    func didAddTask() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
            self.reloadTasksForCurrentGroup()
        }
